	...
	public void onCreate(Bundle savedInstanceState) {
		...
		iv_goodPhoto.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(GoodsAddActivity.this,photoListActivity.class);
		...
		btnAdd.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					/*验证获取物品编号*/
					if(ET_goodNo.getText().toString().equals("")) {
						Toast.makeText(GoodsAddActivity.this, "物品编号输入不能为空!", Toast.LENGTH_LONG).show();
						ET_goodNo.setFocusable(true);
						ET_goodNo.requestFocus();
						return;
					}
					goods.setGoodNo(ET_goodNo.getText().toString());
					/*验证获取物品名称*/ 
					if(ET_goodName.getText().toString().equals("")) {
						Toast.makeText(GoodsAddActivity.this, "物品名称输入不能为空!", Toast.LENGTH_LONG).show();
						ET_goodName.setFocusable(true);
						ET_goodName.requestFocus();
						return;	
					}
					goods.setGoodName(ET_goodName.getText().toString());
					if(goods.getGoodPhoto() != null) {
						//如果图片地址不为空，说明用户选择了图片，这时需要连接服务器上传图片
						GoodsAddActivity.this.setTitle("正在上传图片，稍等...");
						String goodPhoto = HttpUtil.uploadFile(goods.getGoodPhoto());
						GoodsAddActivity.this.setTitle("图片上传完毕！");
						goods.setGoodPhoto(goodPhoto);
					} else {
						goods.setGoodPhoto("upload/noimage.jpg");
					}
					/*验证获取规格型号*/ 
					if(ET_specModel.getText().toString().equals("")) {
						Toast.makeText(GoodsAddActivity.this, "规格型号输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_measureUnit.getText().toString().equals("")) {
						Toast.makeText(GoodsAddActivity.this, "计量单位输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_stockCount.getText().toString().equals("")) {
						Toast.makeText(GoodsAddActivity.this, "库存数量输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_price.getText().toString().equals("")) {
						Toast.makeText(GoodsAddActivity.this, "单价输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_totalMoney.getText().toString().equals("")) {
						Toast.makeText(GoodsAddActivity.this, "金额输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_storeHouse.getText().toString().equals("")) {
						Toast.makeText(GoodsAddActivity.this, "仓库输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_goodMemo.getText().toString().equals("")) {
						Toast.makeText(GoodsAddActivity.this, "备注输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传办公用品信息*/
					GoodsAddActivity.this.setTitle("正在上传办公用品信息，稍等...");
